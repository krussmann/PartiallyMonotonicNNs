    ...
    with open(log_file+'log.txt',"a") as log:
        ...
        if getConfigurations()['counter_example_type'] == "cg" and getConfigurations()['retrain_model']:
            ...
            for epoch in range(0,getConfigurations()['number_of_epochs']):
                ...
                for i_me, batch in enumerate(batches):
                    print("batch: "+str(i_me))
                    ...
                    for res in results:
                        try:
                            ...
                            print("Len of counter examples is " + str(len(counter_examples)))
                            for counter_example in counter_examples:
                    ...
                    if len(counter_batch) != len(batch_labels):
                        ...
                        sys.exit(0)
                    print("len of counter batch is " + str(len(counter_batch)))
                    if (len(counter_batch)>0):
                        ...
                        # Altered by Krussmann
                        train_batch = pd.concat([original_train, counter_batch], ignore_index = True,sort=False)
